...
[`bun install` command](https://bun.sh/docs/cli/install):
...
`hono-sess` accepts these properties in the options object.
...
The default value is a function which uses the `crypto` libraries `randomUUID` to generate IDs.
...
app.use(
  session({
    genid: crypto.randomUUID, // use UUIDs for session IDs
    secret: 'keyboard cat',
  })
)
```
...
app.get('/', (c) => {
  if (c.req.session.views) {
    c.req.session.views++
    return c.html(
      `<p>views: ${c.req.session.views}</p>
       <p>expires in: ${c.req.session.cookie.maxAge / 1000}s</p>`
    )
  } else {
    c.req.session.views = 1
    return c.text('welcome to the session demo. refresh!')
  }
})
```
...
A simple example using `hono-sess` to store page views for a user.

```ts
import { Hono } from 'hono'
import session, { type RequestSessionExtender } from 'hono-sess'

declare module 'hono' {
  interface HonoRequest
    extends RequestSessionExtender<{ views?: Record<string, number> }> {}
}
...
A simple example using `hono-sess` to keep a user log in session.
...
declare module 'hono' {
  interface HonoRequest extends RequestSessionExtender<{ user?: string }> {}
}
...
`hono-sess` when launching your app (`bun start`, in this example):

```sh
$ DEBUG=hono-sess bun start
```

On Windows, use the corresponding command;

```sh
> set DEBUG=hono-sess & bun start
```
